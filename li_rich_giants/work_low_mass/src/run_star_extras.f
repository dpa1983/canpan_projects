...
!     include 'standard_run_star_extras.inc'
      ...
      subroutine extras_controls(s, ierr)
         ...
         s % other_mixing => pavel_mixing_DK          
      ...
      subroutine pavel_mixing_DK(id, ierr)
       ...
       ! if (s% log_surface_luminosity > s% x_ctrl(5)) then
       !    D = s% x_ctrl(2)
       !    delta_K = s% x_ctrl(2)
       !    delta_lgT_mix = s% x_ctrl(4)
       ! end if
       ...
       !    if (dlog((s% r(k))/rsol) < lnr_mix) cycle
                !  next is for  m0.7_fehm1.53_y25_ap4_int from model 1600
       !    if (s% q(k) * s% star_mass < 0.29632d0 .or. dlog((s% r(k))/rsol) < lnr_mix .or. s% xa(h1,k) < 1e-1) cycle
                !  next is for  m0.8_fehm1.53_y25_ap4 from model 1600
       !    if (s% q(k) * s% star_mass < 0.29638d0 .or. dlog((s% r(k))/rsol) < lnr_mix .or. s% xa(h1,k) < 1e-1) cycle
                !  next is for  m0.65_fehm1.53_y25_ap4_extr from model 1700
       !    if (s% q(k) * s% star_mass < 0.29903d0 .or. dlog((s% r(k))/rsol) < lnr_mix .or. s% xa(h1,k) < 1e-1) cycle
                !  next is for  m0.8_fehm2.2_y25_ap4_ngc5466 from model 1900
       !    if (s% q(k) * s% star_mass < 0.33294d0 .or. dlog((s% r(k))/rsol) < lnr_mix .or. s% xa(h1,k) < 1e-1) cycle
       !    if (s% q(k) * s% star_mass < 0.31250d0 .or. s% lnT(k) > lnT_mix .or. s% xa(h1,k) < 1e-1) cycle
                !  next is for  m1.2_fehm0.3_y27_gs98_extr from model 1700
            if (s% q(k) * s% star_mass < 0.269885d0 .or. dlog((s% r(k))/rsol) < lnr_mix .or. s% xa(h1,k) < 1e-1) cycle
            gradr = s% gradr(k)
            ...
            s% D_mix(k) = delta_K*(10d0**(s% log_surface_luminosity-1.8d0))**(4d0/3d0)
       !    s% D_mix(k) = delta_K*thermal_diffusivity
            s% mixing_type(k) = overshoot_mixing

         end do

      end subroutine pavel_mixing_DK
