...
! ***********************************************************************
      ...
      subroutine extras_controls(s, ierr)
         ...
         s % other_mixing => pavel_mixing_nova
      ...
      subroutine pavel_mixing_nova(id, ierr)
         ...
         if (s% log_surface_luminosity < 4d0) return
       ...
       ! has a region with mu inversion been formed?
         ...
         do k = 2, nz ! start at 2 since no mixing across surface
            if ((1d0 - s% q(k)) >= 1d-10 ) cycle
            s% D_mix(k) = 0d0
            s% mixing_type(k) = no_mixing
         end do     

      end subroutine pavel_mixing_nova
